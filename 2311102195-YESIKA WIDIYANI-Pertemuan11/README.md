...
Pastikan Flutter sudah terpasang dan terbaca di terminal.
...
```bash
cd "2311102195-YESIKA WIDIYANI-MODUL-7-FLUTTER"
```
...
Apabila ingin menghentikan aplikasi, tekan `q` pada terminal atau gunakan `CTRL + C`, kemudian ketik `Y` jika diminta konfirmasi.

---

## 8. Langkah Memasukkan ke GitHub

Buka terminal pada folder utama repository, misalnya folder `ABP`, lalu jalankan perintah berikut:

```bash
git status
git add "2311102195-YESIKA WIDIYANI-MODUL-7-FLUTTER/"
git status
git commit -m "Menambahkan tugas Modul 7 Flutter Yesika"
git push origin master
```

Jika branch repository menggunakan `main`, gunakan perintah berikut untuk push:
...
Catatan: jangan gunakan `git add .` apabila masih ada folder lain yang belum jelas statusnya, karena file lain bisa ikut masuk ke commit.
