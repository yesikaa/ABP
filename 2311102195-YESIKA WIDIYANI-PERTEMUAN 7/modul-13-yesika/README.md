...
Di dalam folder `modul-13-yesika` terdapat dua project Laravel. Jalankan salah satu sesuai kebutuhan.

## 6.1 Menjalankan `laravel-modul13`
...
# 7. Langkah Memasukkan Project ke GitHub

Pastikan folder `modul-13-yesika` sudah dipindahkan ke dalam folder repository `ABP`.

Masuk ke folder repo utama:
...
Tambahkan hanya folder modul 13:

```bash
git add modul-13-yesika/
```
...
```bash
git commit -m "Menambahkan tugas Modul 13 Laravel Auth dan Relasi"
```
...
Jika repository kamu memakai branch `main`, gunakan perintah berikut:
...
Jangan menggunakan `git add .` jika masih ada file lain yang belum jelas statusnya, karena file atau folder lain bisa ikut masuk commit tanpa sengaja.

---

# 8. Kesimpulan

Berdasarkan praktikum yang telah dilakukan, modul 13 berhasil menerapkan konsep authentication, middleware, session, dan relasi database pada Laravel. Project `laravel-modul13` menunjukkan penerapan login manual dan relasi One-to-Many antara produk dan varian, sedangkan project `tugas-8` menunjukkan penerapan autentikasi Laravel Breeze serta fitur CRUD untuk inventaris gudang sembako.
